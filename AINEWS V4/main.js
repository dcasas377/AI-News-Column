    ...
    try {
        const response = await fetch("https://api.openai.com/v1/chat/completions", {
            ...
            body: JSON.stringify({
                model: "gpt-3.5-turbo",
                messages: [
                    { role: "system", content: "Summarize this news article in a couple sentences." },
...
async function searchNews(query, category = "") {
    try {
        ...
        if (category) {
            // NewsAPI only supports category in top-headlines, not everything
            // So we warn or fallback gracefully
            console.warn("Category not supported with keyword search via 'everything' endpoint.");
        }
